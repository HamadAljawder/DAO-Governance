// We pull in eveything we need
const Token = artifacts.require("Token")
const Timelock = artifacts.require("Timelock")
const Governance = artifacts.require("Governance")
const Treasury = artifacts.require("Treasury")

// We create our scneario/people
module.exports = async function (callback) {
    const [executor, proposer, voter1, voter2, voter3, voter4, voter5] = await web3.eth.getAccounts()

    //we check the state of the funds (Are they released?) How much funds/what's the block number etc.
    let isReleased, funds, blockNumber, proposalState, vote

    //Delegating tokens from each voter for the proposal.
    ...
    //Check if the funds are released from the treasury
    ...
    // Get the governance contract aling with the encoded function.
